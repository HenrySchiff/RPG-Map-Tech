...
func _ready():
	icon.set_clickable_radius(0.9)

func _process(delta):
	$HeightCaster.raycast()
	#$DirectionVector.basis = movement_state.direction
	#$DirectionVector.look_at(movement_state.velocity + global_position, Vector3(0.001, 1, 0))
